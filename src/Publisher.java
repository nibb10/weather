...
public class Publisher
{
    ...
    private static int run(com.zeroc.Ice.Communicator communicator, String[] args)
    {
        ...
        MonitorPrx monitor = MonitorPrx.uncheckedCast(publisher);
        System.out.println("publishing report events. Press ^C to terminate the application.");      
        try
        {
    ...
    private static Measurement getMeasurement() {
    	String tower = String.valueOf(getRandom(1, 10));
    	float windSpeed = getRandom(20, 100);
    	short windDirection = (short)getRandom(1,10);
    	float temperature = getRandom(10, 40);
    	
    	Measurement m = new Measurement(tower, windSpeed, windDirection, temperature);
    	return m;
    }
